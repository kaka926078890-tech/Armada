...
(async () => {
  try {
    while (true) {
      ...
      if (done) break;
      sseFrames.push(sseDecoder.decode(value));
    }
  } catch { /* aborted */ }
})();
...
const finalRun = await (await fetch(`${base}/api/runs/${created.run.id}`, { headers: H })).json();
...
const sseJoined = sseFrames.join("");
check("SSE status/event frames", /run\.status|run\.event/.test(sseJoined));
