...
- 运行中 steer 续发还没写成用户句时，上一轮完成不再把卡片收成已完成。用户句落地后继续跑；120 秒仍没落地则按原完成收口。只更新中台，扩展和手机不用升号。
- 文件 `@` 菜单按文件名或路径末段全等才点。两个结果都对上时不再点第一项，任务以 `MENTION_CLICK:AMBIGUOUS` 失败。需新的扩展号。
- 消息免打扰改为各看板本机开关。主控拨动不再写进共用 `ui-prefs`，受控看板保持自己的灰点/红点。只更新中台，扩展和手机不用升号。
- Cursor 因 resume 空转把一轮打成异常后，同一条对话里的自动追问会立刻把卡片拉回运行中。只有时间戳的空用户行不会拉回；其它错误原因保持异常。只更新中台，扩展和手机不用升号。
