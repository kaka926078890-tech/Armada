...
- Windows 上带换行的提示词（含快捷提示词）已经写进输入框，但读回把每个换行显示成空行，派发在回车前被 `VERIFY_FAIL:MISMATCH` 拒绝。0.4.45 把这种段落空行折回一次再核对。Mac 上原来的单换行和正文里故意空出的一行仍按原文比对。
- 文件 `@` 菜单按文件名或路径末段全等才点。两个结果都对上时不再点第一项，任务以 `MENTION_CLICK:AMBIGUOUS` 失败。
- 注册和心跳对 9222 的探口与注入前一样重试三次。一次闪断不再把 `cdpReady` 打成 false，手机详情不再误贴「Cursor 在线但无法注入」。
- 运行中 steer 续发还没写成用户句时，上一轮完成不再把卡片收成已完成。用户句落地后继续跑；120 秒仍没落地则按原完成收口。
- 消息免打扰改为各看板本机开关。主控拨动不再写进共用 `ui-prefs`，受控看板保持自己的灰点/红点。
- Cursor 因 resume 空转把一轮打成异常后，同一条对话里的自动追问会立刻把卡片拉回运行中。只有时间戳的空用户行不会拉回；其它错误原因保持异常。
- macOS / Linux 安装扩展时直接解包到 `~/.cursor/extensions`，并改写 `extensions.json`。不再调用 `cursor --install-extension`。

### 操作员注意

Mac 和 Windows 都装扩展 **0.4.45**。中台须 overlay 到含 `REQUIRED_EXTENSION_VERSION=0.4.45` 的包装 hub。目录里没有这个包时，空闲 Reload 不会升级。桌面仍是 **0.1.0**；iOS TestFlight **31**；Android **0.1.14**。

---

## 2026-09-22 — 扩展 0.4.44 · iOS TF 31 · Android 0.1.14 · 桌面 0.1.0

### 新增

- 设置里的快捷提示词改成小标签：点标题编辑正文，角落的 × 删除。中台已在当前安装包里；手机要装 TestFlight **31** / Android **0.1.14**。
- 手机详情可以点开回复里的文本链接，以及中台「已编辑」文件，直接读被控工作区内容。TestFlight **30** / Android **0.1.13** 还没有这个入口。

### 修复

- 中台编辑快捷提示词时，标题不再被设置弹窗切掉上方空隙。

### 操作员注意

装 iOS TestFlight **31**，Android **0.1.14**（versionCode 15）。扩展仍是 **0.4.44**；桌面仍是 **0.1.0**。中台已 overlay，不用 Reload。
