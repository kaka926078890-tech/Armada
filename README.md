...
桌面 App：**创建舰队**默认勾选「开放到局域网」，同网其他 Armada 的加入页会列出该舰队（点一下即加入）。不勾选则不广播，仍可复制 `armada://join?…`。组播被拦时列表为空，粘贴链接仍可用。
...
4. **安装扩展** `armada-agent` ≥ 0.4.18（macOS 收口须 ≥ 0.4.18；Windows 绑定与续聊收口必须 ≥ 0.4.18）  
...
| `armada-agent-0.4.18.vsix` | 中台 `extension\armada-agent-0.4.18.vsix`，或 Windows 自己 `npm install && npx tsup && npx --yes @vscode/vsce package --no-dependencies`（必须 ≥ 0.4.18） | 0.4.11 Reload 会把 `ext_seq` 重数到已占用号段，hub 丢掉 `stop`。0.4.12 时钟播种 seq。**0.4.16 续聊 fromEnd 会清掉 hub 签发的 generation_id**，jsonl 已 `turn_ended` 仍停在运行中；0.4.17 保留该 gen。**0.4.17 只在 Windows 合成 stop**，macOS 对话已结束仍会停在运行中；0.4.18 全平台合成。 |
...
2. 左侧扩展 → `...` → **Install from VSIX** → 选中 `armada-agent-0.4.18.vsix`。
3. 装完先不要关。
...
| 一直「待本机回车」但黄字是「绑定中」，约 1 分钟后进异常 | **受控**：Windows 须装 **armada-agent ≥ 0.4.18** 并 Reload。0.4.10 扫描窗 20s 会 BIND_TIMEOUT。 |
| 本机对话已结束，看板仍「运行中」 | **受控**：须 ≥ 0.4.18。0.4.17 只在 Windows 从 jsonl 合成 `stop`；macOS 仍只赌 Cursor `stop` hook。日志：`stop synthesized` / `adopt r-…`。Hub 须把 `status: success` 收成 completed。 |
