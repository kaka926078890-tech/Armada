...
| 扩展 armada-agent | **0.4.37** | `extension/package.json` |
...
受控端也可 clone 后自己打包。当前包名 `armada-agent-0.4.37.vsix`。
...
4. **安装扩展** `armada-agent` **0.4.37**（能力下限 ≥ 0.4.19）  
...
| `armada-agent-0.4.37.vsix` | 中台 `extension\armada-agent-0.4.37.vsix`，或 Windows 自己 `npm install && npx tsup && npx --yes @vscode/vsce package --no-dependencies`（当前 **0.4.37**，能力下限 ≥ 0.4.19） | 0.4.11 Reload 会把 `ext_seq` 重数到已占用号段，hub 丢掉 `stop`。0.4.16 续聊 fromEnd 会清掉 hub 签发的 `generation_id`。**0.4.17 只在 Windows 合成 stop**；**0.4.18 起全平台合成**。请不要用更旧的 vsix。 |
...
2. 左侧扩展 → `...` → **Install from VSIX** → 选中 `armada-agent-0.4.37.vsix`。
3. 装完先不要关。
